...
/**
 * @brief     : 根据给定的选项提供静态文件。文件也可以进行 gzip 压缩，包括目录索引。所有压缩文件都必须以 .gz 结尾，并且不得存在没有扩展名的同名文件，否则将优先;见mg_http_serve_file（）
 * @return     {*}
 * @note      : 
 */
 ...
 * @brief     : 创建一个HTTP服务
 * @Author    : lzy
 * @Date      : 2024-02-01 19:53
 * @param      {mg_mgr} *mgr
 * @param      {char} *url 附加一个本机的IP和监听的端口，http://0.0.0.0:8000
 * @param      {mg_event_handler_t} fn 回调函数，当有监听到有连接进来的时候，就执行回调函数
 * @param      {void} *fn_data 传给回调函数的参数
 * @return     {*}
 * @note      : 
 */
 ...
 * @brief     : 初始化事件管理器结构
 * @Author    : lzy
 * @Date      : 2024-02-01 19:15
 * @param      {mg_mgr} *mgr指向需要初始化的结构的指针mg_mgr
 * @return     {*}
 * @note      :  
 *  将活动连接列表设置为 NULL
    设置 IPv4 和 IPv6 的默认 DNS 服务器
    设置默认 DNS 查找超时
 */
void mg_mgr_init(struct mg_mgr *mgr) {
  memset(mgr, 0, sizeof(*mgr));
#if MG_ENABLE_EPOLL
  if ((mgr->epoll_fd = epoll_create1(EPOLL_CLOEXEC)) < 0) // epoll_create1(EPOLL_CLOEXEC) 是一个用于创建 epoll 实例的系统调用，它用于异步事件通知。 EPOLL_CLOEXEC 是一个标志，它告诉操作系统在创建 epoll 实例时将其设置为 close-on-exec（CLOEXEC）模式。以提高安全性，防止在进程执行过程中不必要地泄漏 epoll 文件描述符。这对于多进程或多线程应用程序中的事件通知机制非常有用。
...
/**
 * @brief     : 从 C 字符串创建 Mongoose 字符串（可以是非 NULL 结尾，长度为 在 ） 中指定。
 * @param      {char} *s     指向要存储在 created 中的字符串的指针 mg_str
 * @param      {size_t} n    字符串长度
 * @return     {*}
 * @note      : 此函数不会重复提供的字符串， 但将指针存储在创建的结构中。
 */
...
/**
 * @brief     : 不分大小写的比较两个以NULL结束的字符串
 * @Author    : lzy
 * @Date      : 2024-02-01 19:55
 * @param      {char} *s1 指向字符串的指针
 * @param      {char} *s2 指向字符串的指针
 * @return     {*}
 * @note      : 如果返回0则说明这两个字符串相等，如果大于0则s1>s2,如果小于0则s1<s2
 */
